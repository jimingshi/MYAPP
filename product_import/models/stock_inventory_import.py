...
class stock_inventory_import(models.Model):
    ...
    def _handle_inventory(self, sheet):
        ...
        location_id=self.env['stock.location'].search([('name', '=', sheet.cell(1, 2).value)]).id
        ...
        for i in range(1, sheet.nrows):
            product_template_name = sheet.cell(i, 3).value.strip()
            ...
            uom_name = sheet.cell(i, 5).value

            location_name = sheet.cell(i, 6).value

            prodlot_name = sheet.cell(i, 7).value
            ...
            vals1={
                'line_ids':[(0, False, {
                    ...
                    'product_code': code,
                    'product_uom_id': self.env['product.uom'].search([('name', '=', uom_name)]).id,
                    'product_qty': sheet.cell(i, 10).value,
                    'location_id': self.env['stock.location'].search([('name', '=', location_name)]).id,
                    'location_name': location_name,
                    'partner_id': self.env['res.partner'].search([('name', '=', sheet.cell(i, 8).value)]).id,
